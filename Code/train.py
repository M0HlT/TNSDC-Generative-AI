...
optimizer = tf.keras.optimizers.Adam(learning_rate=CONFIG.learning_rate, decay=CONFIG.learning_rate_decay)
model = create_model(total_words=total_words, hidden_size=CONFIG.hidden_size,num_steps=CONFIG.number_of_words, optimizer=optimizer)
print(model.summary())

checkpointer = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join('/Users/mohits/Downloads/language-modeling/model/checkpoint/', 'model-{epoch:02d}.keras'), verbose=1)
...
model.save(os.path.join ('/Users/mohits/Downloads/language-modeling/model/', 'model.keras'))
